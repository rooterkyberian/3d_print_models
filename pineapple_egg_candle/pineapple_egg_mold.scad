...
module space(n) {
    translate([n *spacing, 0, 0]) children();
}
...
module funnel_ring(thickness=ring_thickness, tolerance=ring_tolerance) {
    ...
    difference() {
        ...
        translate([0,0,-1]) cylinder(h=funnel_ring_h*2, d=funnel_d+tolerance);
    }
}

module inner_funnel() {
    cylinder(h=funnel_h+2, d=funnel_inner_d);
}
...
module whole_mold() {
    difference() {
        union() {
            base();
            translate([0,0,-mold_thickness]) hull() egg(egg_h+2*mold_thickness, egg_d+2*mold_thickness);
